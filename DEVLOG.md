...
- **Session 17 (Dev Auth Bypass — KioskTriage):** Added a dev-only auto-authentication `useEffect` block in `frontend/src/pages/kiosk/KioskTriage.jsx`. When the component mounts without a JWT (e.g. direct navigation to `/kiosk/triage`), it calls `POST /api/auth/otp/request` for account `1000000001`, fetches the OTP via `GET /api/dev/peek-otp?account_number=1000000001`, and completes verification automatically. `jwt` and `failedTxSummary` are now held in React state (not just router state) so the bypass sets them directly via `setJwt` / `setFailedTxSummary` — triggering a re-render in-place without any `navigate()` call. A second `useEffect` watching `jwt` clears `devBooting` once the token arrives. While the bypass is in flight a neumorphic loading spinner renders (`devBooting` state flag); if the peek endpoint is absent or any step fails, `devBooting` is set to `false` and the normal triage screen renders. Added `GET /api/dev/kiosk-bypass` — a single-call endpoint in `backend/src/index.js` (guarded by `NODE_ENV !== 'production'`) that performs account lookup, OTP generation, OTP consumption, CBS failed-tx query, JWT issuance, and Redis session creation in one request, eliminating the async race condition of the previous two-step peek-then-verify flow. `GET /api/dev/peek-otp` is retained (legacy, useful for curl-based manual testing). **The bypass block in `KioskTriage.jsx` and both dev endpoints in `index.js` must be removed before production deployment.**
